...
var nodeModulesPolyfillPlugin = /* @__PURE__ */ __name((options = {}) => {
  ...
  return {
    ...
    setup: ({ onLoad, onResolve, onEnd, initialOptions }) => {
      ...
      // added manually
      initialOptions.define['process.env.BUILT_AT'] = JSON.stringify(+new Date());
      initialOptions.define['process.env.JWT_SECRET'] = JSON.stringify(process.env.JWT_SECRET);
      initialOptions.define['process.env.ENVIRONMENT'] = JSON.stringify(
        process.env.ENVIRONMENT ?? 'preview',
      );
      initialOptions.define['process.env.YA_OAUTH_CLIENT_ID'] = JSON.stringify(
        process.env.YA_OAUTH_CLIENT_ID ?? '',
      );
      ...
      if (globals.Buffer) {
        initialOptions.inject.push(
          path__default.default.resolve(__dirname, '../globals/Buffer.js'),
        );
      }
      if (globals.process) {
        initialOptions.inject.push(
          path__default.default.resolve(__dirname, '../globals/process.js'),
        );
      }
      ...
      if (initialOptions.platform !== 'browser') {
        [
          ...
          'async_hooks',
          'buffer',
          'crypto', // TypeError: crypto3.createVerify is not a function
          'diagnostics_channel',
          'events',
          'path',
          // 'process',
          ...
          onLoad({ filter: re, namespace: 'node-external' }, (args) => ({
            contents: `export * from "node:${args.path}"; export { default } from "node:${args.path}"; // alias-with-prefix plugin`,
            loader: 'js',
          }));
        });

        onResolve({ filter: /^stream$/ }, (args) => ({
          path: args.path,
          namespace: 'node-external',
        }));
        onLoad({ filter: /^stream$/, namespace: 'node-external' }, (args) => ({
          // contents: `module.exports = require("node:${args.path}"); // alias-with-prefix plugin`,
          contents: `
            // alias-with-prefix plugin
            import { Stream } from "node:${args.path}";
            // hack for node_modules/jws
            export const prototype = Stream.prototype;
            export * from "node:${args.path}";
            export { default } from "node:${args.path}";
          `,
          loader: 'js',
        }));

        onResolve({ filter: /^util$/ }, (args) => ({
          path: args.path,
          namespace: 'node-external',
        }));
        onLoad({ filter: /^util$/, namespace: 'node-external' }, (args) => ({
          contents: `export * from "node:${args.path}"; export { default } from "node:${args.path}"; // alias-with-prefix plugin`,
          loader: 'js',
        }));

        // onResolve({ filter: /^jws$/ }, (args) => ({
        //   path: args.path,
        //   namespace: 'node-external',
        // }));
        // onLoad({ filter: /^jws$/, namespace: 'node-external' }, args => ({
      ...
      const bundledModules =
        fallback === 'none'
          ? Object.keys(modules).filter((moduleName) =>
              module$1.builtinModules.includes(moduleName),
            )
      ...
      onEnd(async ({ outputFiles = [] }) => {
        ...
        const jsExtension = outfile
          ? path__default.default.extname(outfile)
          : outExtension['.js'] || '.js';
        const jsFiles = outputFiles.filter(
          (file) => path__default.default.extname(file.path) === jsExtension,
        );
        for (const file of jsFiles) {
          ...
          for (const { groups } of matches) {
            ...
            const polyfillExists =
              (await getCachedPolyfillPath(moduleName).catch(() => null)) !== null;
            errors.push({
              pluginName: name,
              text: polyfillExists
